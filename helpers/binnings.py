...
class FixedBordersBinning(Binning):
    ...
    def __init__(self, p, y, c, bin_borders):
        ...
        if len(bin_borders) < 2:
            raise ValueError("Too few bin borders")

        bin_borders[0] = 0
        bin_borders[-1] = 1

        self.bin_borders = np.array(bin_borders)
        super().__init__(p, y, c)

    def split_indices_into_bins(self):

        indices = np.array(range(len(self.p)))
        conditions = [(self.p >= self.bin_borders[bin_idx]) & (self.p < self.bin_borders[bin_idx+1]) for bin_idx in range(len(self.bin_borders)-1)]
        bins = np.array([indices[condition] for condition in conditions], dtype=object)
